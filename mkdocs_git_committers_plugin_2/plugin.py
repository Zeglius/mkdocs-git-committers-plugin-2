    ...
    def get_contributors_to_file(self, path):
        ...
        if self.config["gitlab_repository"]:
            ...
            url = (
                self.gitlaburl
                + "/projects/"
                + str(self.config["gitlab_repository"])
        ...
        if r.status_code == 200:
            ...
            for commit in res:
                ...
                elif commit["author_name"]:
                    # If author is not already in the list of authors
                    if commit["author_name"] not in [
                        ...
                        if commit["author_name"] in self.gitlabauthors_cache:
                            authors.append(
                                {
                                    "login": self.gitlabauthors_cache[
                                        commit["author_name"]
                                    ]["username"],
                                    "name": commit["author_name"],
                                    "url": self.gitlabauthors_cache[
                                        commit["author_name"]
                                    ]["web_url"],
                                    "avatar": self.gitlabauthors_cache[
                                        commit["author_name"]
                                    ]["avatar_url"],
                                }
                            )
                            ...
                            url = (
                                self.gitlaburl
                                + "/users?search="
                                + requests.utils.quote(commit["author_name"])
                            )
                            r = requests.get(url=url, headers=self.auth_header)
                            if r.status_code == 200:
                                res = r.json()
